...
import { onRouteChange, modifyRoutes, scrollHandler } from 'gatsby-browser'
...
function render() {
  ...
  const handle = scrollHandler || function() { return true }
  const router = (
    <Router
      history={browserHistory}
      routes={routes}
      render={applyRouterMiddleware(useScroll(handle))}
    />
  )
